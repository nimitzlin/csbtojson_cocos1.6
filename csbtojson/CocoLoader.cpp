...
namespace cocostudio{
	...
	int CocoLoader::dump(std::string path)
 	{
		std::ofstream t;  
		t.open(path, ios::binary);      // open input file  
		if (!t)
		{
			std::cout<<"out put file failed "<<path<<std::endl;
		}
		//t.write((char*)m_pObjectDescArray, m_pFileHeader->m_nDataSize);

		char*	pDestBuff  = new char[(m_pFileHeader->m_nCompressSize)+10000];
		memset(pDestBuff, 0, (m_pFileHeader->m_nCompressSize)+10000);

		uLongf		dwSrcSize  = m_pFileHeader->m_nCompressSize +10000;
		uLongf		dwDestSize  = m_pFileHeader->m_nDataSize;
		int a=compress((Bytef*)pDestBuff,&dwSrcSize,(Bytef*)m_pObjectDescArray,dwDestSize);
		m_pFileHeader->m_nCompressSize = (uint32_t)dwSrcSize;
		t.write((char*)m_pFileHeader, sizeof(stCocoFileHeader));
		t.write((char*)pDestBuff, dwSrcSize);
